...
{-| 0 that conforms to any number type like `N0able Integer.Signed Possibly` or `N0able Natural.N1Up Possibly`
-}
...
{-| Intended for package authors. If the number is not 0, change it using a given function based on its current value.
    ...
    integerToNatural : N0able Integer.Signed n0 -> N0able Natural.N1Up n0
    integerToNatural =
...
{-| Usually used to take a `N0able ... Never` argument and make a `N0able ... freeVariable_` value out of it
    ...
    natural1UpToIntegerSigned : N0able Natural.N1Up Never -> N0able Integer.Signed freeVariable_
...
-}
n0Adapt : (n0PossiblyOrNever -> adaptedN0PossiblyOrNever) -> (N0able not0 n0PossiblyOrNever -> N0able not0 adaptedN0PossiblyOrNever)
...
with the wrapped non-0 number representation.
    ...
    Stack.fromList [ Natural.n1, N0able.n0 ]
        |> Stack.map N0able.toEmptiableNot0
        |> Stack.fills
    --> Stack.fromList [ Natural.n1 ]
